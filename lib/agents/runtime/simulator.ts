...
import { assertApprovedTextModel, modelPolicyVersion } from "./model-policy";
...
export function simulateAgentRun(request: RuntimeRequest, controls: RuntimeControls = simulationControls): RuntimeResult {
  ...
  if (request.provider !== "openai") throw new Error("This provider is available as an adapter but is not approved in the active CEO model policy.");
  const approvedModel = assertApprovedTextModel(request.modelAlias, request.mainAgent);
  add("policy", "passed", `CEO model policy v${modelPolicyVersion} approved ${approvedModel.id}; ${request.tokenBudget.toLocaleString()} token ceiling, ${request.timeoutMs / 1000}s timeout and ${request.maxRetries} retries.`);
  ...
  const cost = (input / 1_000_000) * approvedModel.inputUsdPerMillion + (output / 1_000_000) * approvedModel.outputUsdPerMillion;
