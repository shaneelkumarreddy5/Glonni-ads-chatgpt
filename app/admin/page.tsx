...
function AgentWorkspace({ agent, onBack, action }: { agent: AgentDirectoryItem; onBack: () => void; action: (message: string) => void }) {
  ...
  const visibleWorkspaceTabs = isSupportTeamLead ? supportAgentWorkspaceTabs : isFraudRiskAgent ? fraudRiskAgentWorkspaceTabs : isComplianceAgent ? complianceAgentWorkspaceTabs : isAdsOperationsAgent ? adsOperationsAgentWorkspaceTabs : agentWorkspaceTabs;
  ...
  return <>
    ...
    : tab === "Tasks" && isSupportTeamLead ? <SupportTasks/> : tab === "Tasks" && isFraudRiskAgent ? <RiskTasks/> : tab === "Tasks" && isComplianceAgent ? <ComplianceTasks/> : tab === "Tasks" && isAdsOperationsAgent ? <AdsTasks/> : tab === "Tasks" ? <AgentEmptyPanel icon={CheckCircle2} title="Agent Tasks" note="Running, waiting, completed, failed and escalated assignments will appear here." />
    : tab === "Reports" && isSupportTeamLead ? <SupportReports/> : tab === "Reports" && isFraudRiskAgent ? <RiskReports/> : tab === "Reports" && isComplianceAgent ? <ComplianceReports/> : tab === "Reports" && isAdsOperationsAgent ? <AdsReports/> : tab === "Reports" ? <AgentEmptyPanel icon={SlidersHorizontal} title="Agent Reports" note="Daily, weekly and performance reports will appear here after activation." />
    : tab === "Approvals" && isSupportTeamLead ? <SupportApprovals action={action}/> : tab === "Approvals" && isFraudRiskAgent ? <RiskApprovals action={action}/> : tab === "Approvals" && isComplianceAgent ? <ComplianceApprovals action={action}/> : tab === "Approvals" && isAdsOperationsAgent ? <AdsApprovals action={action}/> : tab === "Approvals" ? <AgentApprovalsShell agent={agent} action={action}/>
    : tab === "Automations" && isFraudRiskAgent ? <RiskAutomations action={action}/> : tab === "Automations" && isComplianceAgent ? <ComplianceAutomations action={action}/> : tab === "Automations" && isAdsOperationsAgent ? <AdsAutomations action={action}/> : tab === "Automations" ? <SupportAutomations action={action}/>
    : tab === "Activity History" && isSupportTeamLead ? <SupportActivityHistory/> : tab === "Activity History" && isFraudRiskAgent ? <RiskActivityHistory/> : tab === "Activity History" && isComplianceAgent ? <ComplianceActivity/> : tab === "Activity History" && isAdsOperationsAgent ? <AdsActivityHistory/> : tab === "Activity History" ? <AgentActivityHistoryShell agent={agent} instructions={instructions}/>
    : tab === "Connections" && isSupportTeamLead ? <SupportConnections action={action}/> : tab === "Connections" && isFraudRiskAgent ? <RiskConnections action={action}/> : tab === "Connections" && isComplianceAgent ? <ComplianceConnections action={action}/> : tab === "Connections" && isAdsOperationsAgent ? <AdsConnections action={action}/> : tab === "Connections" ? <AgentEmptyPanel icon={Link2} title="Connections" note="Only approved email, provider, API and service connections will be available here." />
    : isSupportTeamLead ? <SupportSettings/> : isFraudRiskAgent ? <RiskSettings/> : isComplianceAgent ? <ComplianceSettings/> : isAdsOperationsAgent ? <AdsSettings/> : <AgentPermissionsHierarchy agent={agent}/>}
