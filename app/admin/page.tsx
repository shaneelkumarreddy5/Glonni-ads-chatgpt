...
function AgentWorkspace({ agent, onBack, action }: { agent: AgentDirectoryItem; onBack: () => void; action: (message: string) => void }) {
  ...
  return <>
    ...
    : tab === "Approval Queue" ? <ContentApprovalQueue action={action}/>
    ...
    : tab === "Connections" && isSupportTeamLead ? <SupportConnections action={action}/> : tab === "Connections" && isFraudRiskAgent ? <RiskConnections action={action}/> : tab === "Connections" && isComplianceAgent ? <ComplianceConnections action={action}/> : tab === "Connections" && isAdsOperationsAgent ? <AdsConnections action={action}/> : tab === "Connections" && isOfferwallAgent ? <OfferConnections action={action}/> : tab === "Connections" && isAffiliateShopAgent ? <AffiliateConnections action={action}/> : tab === "Connections" && isContentManagerAgent ? <ContentConnections action={action}/> : tab === "Connections" ? <AgentEmptyPanel icon={Link2} title="Connections" note="Only approved email, provider, API and service connections will be available here." />
    : isSupportTeamLead ? <SupportSettings/> : isFraudRiskAgent ? <RiskSettings/> : isComplianceAgent ? <ComplianceSettings/> : isAdsOperationsAgent ? <AdsSettings/> : isOfferwallAgent ? <OfferSettings/> : isAffiliateShopAgent ? <AffiliateSettings/> : isContentManagerAgent ? <ContentSettings/> : <AgentPermissionsHierarchy agent={agent}/>}
