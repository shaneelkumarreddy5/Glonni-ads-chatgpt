...
function SettingsSecurity({action}:{action:(message:string)=>void}){
  ...
  return <>
    ...
    {tab==="Integrations"&&<section className="rounded-2xl border bg-white p-5"><h2 className="font-bold">Credentials, APIs & webhooks</h2><p className="text-xs text-slate-500">Values remain masked; production secrets must live in a secure server-side vault, never this browser</p><div className="mt-5 grid gap-4 lg:grid-cols-2">{[["Supabase backend","Not connected","Environment placeholders only"],["Reward provider postbacks","Mock","Signature verification required"],["Affiliate callbacks","Mock","Allowlist and replay protection required"],["Support channels","Mock","No messages are sent"],["AI agent model","Not connected","Scoped tools and spend limit required"],["Payment / payout provider","Not connected","Dual approval required"]].map(x=><article key={x[0]} className="flex items-center gap-3 rounded-xl border p-4"><Wifi size={18} className="text-violet-600"/><div><b className="text-xs">{x[0]}</b><p className="text-[10px] text-slate-400">{x[2]}</p></div><span className="ml-auto rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-600">{x[1]}</span><button onClick={()=>setConfirm("rotate")} className="text-xs font-bold text-violet-600">Manage</button></article>)}</div><div className="mt-5 rounded-xl bg-amber-50 p-4 text-xs leading-5 text-amber-900"><ShieldAlert size={16} className="mb-1"/>The UI never displays or stores real API keys. Secret creation, reveal, rotation and revocation require backend authorization, re-authentication and audit logging.</div></section>}
            ...
            <div className="mb-5" key={group.label ?? "main"}>
              ...
              <div className="space-y-1">
                {group.items.map((item) => {
                  const enabled = item.label === "Dashboard" || item.label === "Users" || item.label === "Wallet & Transactions" || item.label === "Withdrawals" || item.label === "KYC Verification" || item.label === "Fraud & Risk" || item.label === "Ad Networks" || item.label === "Surveys" || item.label === "App Install Offers" || item.label === "Games" || item.label === "Shop & Earn" || item.label === "Referrals" || item.label === "Content" || item.label === "Support Centre" || item.label === "Reports" || item.label === "Settings & Security";
