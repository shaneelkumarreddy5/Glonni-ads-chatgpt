...
import { SupportActivityHistory, SupportApprovals, SupportAutomations, SupportConnections, SupportReports, SupportSettings, SupportSubagents, SupportTasks, SupportTeamLeadOverview, SupportTickets } from "./support-team-lead-agent";
import { FraudRiskOverview, FraudRiskSubagents, RiskActivityHistory, RiskAppeals, RiskApprovals, RiskAutomations, RiskCases, RiskConnections, RiskReports, RiskReviewQueue, RiskRules, RiskSettings, RiskTasks } from "./fraud-risk-agent";
import { ComplianceActivity, ComplianceApprovals, ComplianceAutomations, ComplianceConnections, ComplianceFinancialRecords, ComplianceKycCases, ComplianceOverview, ComplianceReports, ComplianceReviews, ComplianceSettings, ComplianceSubagents, ComplianceTasks } from "./compliance-kyc-finance-agent";
import { AdsActivityHistory, AdsApprovals, AdsAutomations, AdsCampaigns, AdsConnections, AdsOperationsOverview, AdsOperationsSubagents, AdsProviders, AdsReports, AdsRevenueReconciliation, AdsRewardTracking, AdsSettings, AdsTasks } from "./ads-operations-agent";
import { AppInstallOffers, ConversionReconciliation, OfferActivity, OfferApprovals, OfferAutomations, OfferConnections, OfferProviders, OfferReports, OfferSettings, OfferSurveys, OfferTaskTracking, OfferTasks, OfferwallOverview } from "./offerwall-tasks-agent";
import { AffiliateActivity, AffiliateApprovals, AffiliateAutomations, AffiliateCommissions, AffiliateConnections, AffiliateProviders, AffiliateReports, AffiliateSettings, AffiliateShopOverview, AffiliateTasks, ClickTracking, ProductCatalog, ProductFeeds, StoresDeals } from "./affiliate-shop-agent";
import { CampaignCopy, ContentActivity, ContentApprovals, ContentAutomations, ContentBriefs, ContentCalendar, ContentConnections, ContentManagerOverview, ContentReports, ContentSettings, ContentSources, ContentTasks, ContentApprovalQueue } from "./content-manager-agent";
import { BrandReview, CreativeActivity, CreativeApprovals, CreativeAutomations, CreativeConnections, CreativeProductionOverview, CreativeReports, CreativeRequests, CreativeSettings, CreativeTasks, CreativeApprovalQueue, DesignLibrary, VideoLibrary } from "./creative-production-agent";
...
const agentDirectory: AgentDirectoryItem[] = [
  ...
  { number: "06", name: "Ads Operations Agent", scope: "Watch & Earn operations", icon: MonitorPlay, subagents: ["Provider & Campaign Operations", "Postback, Reward & Revenue Reconciliation"], operations: ["Ad Providers", "Campaigns", "Reward Tracking", "Revenue Reconciliation", "API Connections"] },
  { number: "07", name: "Offerwall & Tasks Agent", scope: "Surveys and app-install tasks", icon: PackageCheck, subagents: ["Provider & Offer Operations", "Task, Conversion & Reward Reconciliation"], operations: ["Offer Providers", "Surveys", "App-Install Offers", "Task Tracking", "Conversion Reconciliation", "API Connections"] },
...
type AgentWorkspaceTab = "Overview" | "Chat & Instruct" | "Subagents" | "Support Tickets" | "Risk Cases" | "Review Queue" | "Appeals" | "Risk Rules" | "KYC Cases" | "Compliance Reviews" | "Financial Records" | "Ad Providers" | "Campaigns" | "Reward Tracking" | "Revenue Reconciliation" | "Offer Providers" | "Surveys" | "App-Install Offers" | "Task Tracking" | "Conversion Reconciliation" | "Providers" | "Product Feeds" | "Product Catalog" | "Stores & Deals" | "Click Tracking" | "Commissions" | "Content Calendar" | "Content Briefs" | "Campaign Copy" | "Approval Queue" | "Content Sources" | "Creative Requests" | "Design Library" | "Video Library" | "Brand Review" | "Publishing Calendar" | "Scheduled Posts" | "Social Conversations" | "Content Performance" | "Advertising Campaigns" | "Audiences" | "Budgets" | "A/B Tests" | "Campaign Approvals" | "Saved Instructions" | "Tasks" | "Reports" | "Approvals" | "Automations" | "Activity History" | "Connections" | "Settings";
...
const complianceAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Subagents", "KYC Cases", "Compliance Reviews", "Financial Records", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
const adsOperationsAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Subagents", "Ad Providers", "Campaigns", "Reward Tracking", "Revenue Reconciliation", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
const offerwallAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Subagents", "Offer Providers", "Surveys", "App-Install Offers", "Task Tracking", "Conversion Reconciliation", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
const affiliateShopAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Subagents", "Providers", "Product Feeds", "Product Catalog", "Stores & Deals", "Click Tracking", "Commissions", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
const contentManagerAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Content Calendar", "Content Briefs", "Campaign Copy", "Approval Queue", "Content Sources", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
const creativeProductionAgentWorkspaceTabs: AgentWorkspaceTab[] = ["Overview", "Chat & Instruct", "Subagents", "Creative Requests", "Design Library", "Video Library", "Brand Review", "Approval Queue", "Saved Instructions", "Tasks", "Reports", "Approvals", "Automations", "Activity History", "Connections", "Settings"];
...
function ChiefOperationsOverview({ instructionsCount, onOpen, action }: { instructionsCount: number; onOpen: (tab: AgentWorkspaceTab) => void; action: (message: string) => void }) {
  ...
  return <div className="space-y-5">
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[["Status","Configured"],["Departments","13"],["Saved instructions",String(instructionsCount)],["Live automations","0"]].map(([label,value]) => <article key={label} className="rounded-2xl border bg-white p-5 shadow-sm"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 text-xl font-bold">{value}</p></article>)}</section>
    <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <article className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="flex items-center border-b p-5"><div><h2 className="font-bold">Department Agents</h2><p className="mt-1 text-xs text-slate-500">Operational status for the 13 agents reporting to Chief Operations.</p></div><span className="ml-auto rounded-full bg-amber-50 px-3 py-1 text-[9px] font-bold text-amber-700">ACTIVITY PAUSED</span></div><div className="grid gap-3 p-4 sm:grid-cols-2">{chiefDepartmentPreview.map((department) => <div key={department.number} className="rounded-xl border p-4"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-violet-50 text-[10px] font-bold text-violet-700">{department.number}</span><div className="min-w-0"><b className="block truncate text-xs">{department.name}</b><p className="mt-1 text-[10px] text-slate-400">{department.report}</p></div><span className={`ml-auto shrink-0 rounded-full px-2 py-1 text-[9px] font-bold ${department.state === "Ready" ? "bg-emerald-50 text-emerald-700" : department.state === "Review" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{department.state}</span></div></div>)}</div></article>
      <div className="space-y-5"><article className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-center"><h2 className="font-bold">CEO Approvals</h2><span className="ml-auto rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-600">NO PENDING REQUESTS</span></div><div className="mt-4 space-y-3">{previewApprovals.map(([title,source,state]) => <button key={title} onClick={() => action(`${title} has no active approval request`)} className="w-full rounded-xl border p-3 text-left"><b className="block text-xs">{title}</b><p className="mt-1 text-[10px] text-slate-400">{source}</p><span className="mt-2 inline-flex rounded-full bg-amber-50 px-2 py-1 text-[9px] font-bold text-amber-700">{state}</span></button>)}</div></article><article className="rounded-2xl border border-violet-200 bg-violet-50 p-5"><ShieldCheck className="text-violet-600" size={19}/><h2 className="mt-3 font-bold text-violet-950">CEO authority</h2><p className="mt-2 text-xs leading-5 text-violet-900">The Chief Operations Agent may collect evidence and recommend an action. Budgets, payouts, partnerships and policy exceptions always require CEO approval.</p></article></div>
    </section>
...
function AgentWorkspace({ agent, onBack, action }: { agent: AgentDirectoryItem; onBack: () => void; action: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center">
      ...
      <div><div className="flex flex-wrap items-center gap-2"><span className="text-xs font-bold text-violet-600">{agent.number}</span><h1 className="text-2xl font-bold tracking-tight md:text-[28px]">{agent.name}</h1><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold ${isChiefOperations ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{isChiefOperations ? "CONFIGURED" : "SETUP REQUIRED"}</span></div><p className="mt-1 text-sm text-slate-500">{agent.scope} · Secure agent workspace</p></div>
      <div className="lg:ml-auto flex flex-wrap gap-2"><button onClick={() => setTab("Chat & Instruct")} className="flex h-10 items-center gap-2 rounded-xl border bg-white px-4 text-xs font-bold text-violet-700"><Bot size={15}/>Chat & Instruct</button><button onClick={() => action(`${agent.name} is not active. Complete its permissions, connections and operating instructions first.`)} className="flex h-10 items-center gap-2 rounded-xl bg-violet-600 px-4 text-xs font-bold text-white"><MonitorPlay size={15}/>Run Agent</button></div>
    </div>
    ...
    <div className="admin-scroll mb-5 flex gap-1 overflow-x-auto rounded-xl border bg-white p-1" role="tablist" aria-label={`${agent.name} workspace`}>
      {visibleWorkspaceTabs.map((item) => <button key={item} role="tab" aria-selected={tab === item} onClick={() => setTab(item)} className={`shrink-0 rounded-lg px-4 py-2.5 text-xs font-bold ${tab === item ? "bg-violet-600 text-white shadow-sm" : "text-slate-500 hover:bg-slate-50"}`}>{item}</button>)}
    </div>

    {tab === "Overview" && isChiefOperations ? <ChiefOperationsOverview instructionsCount={instructions.length} onOpen={setTab} action={action}/> : tab === "Overview" && isSupportTeamLead ? <SupportTeamLeadOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isFraudRiskAgent ? <FraudRiskOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isComplianceAgent ? <ComplianceOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isAdsOperationsAgent ? <AdsOperationsOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isOfferwallAgent ? <OfferwallOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isAffiliateShopAgent ? <AffiliateShopOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isContentManagerAgent ? <ContentManagerOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isCreativeProductionAgent ? <CreativeProductionOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isSocialMediaAgent ? <SocialMediaOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" && isPerformanceMarketingAgent ? <PerformanceMarketingOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" ? <div className="space-y-5">
      ...
      <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><LayoutDashboard className="text-violet-600" size={19}/><div><h2 className="font-bold">Department workspace</h2><p className="text-xs text-slate-500">Operational areas and connected services for this agent.</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{agent.operations.map((operation, index) => <button key={operation} onClick={() => action(`${operation} is not configured yet`)} className="flex min-h-16 items-center gap-3 rounded-xl border p-4 text-left hover:border-violet-300 hover:bg-violet-50"><span className="grid h-7 w-7 place-items-center rounded-lg bg-violet-100 text-[10px] font-bold text-violet-700">{String(index + 1).padStart(2,"0")}</span><b className="text-xs">{operation}</b><ChevronRight className="ml-auto text-slate-300" size={15}/></button>)}</div></section>
    ...
    </section> : tab === "Subagents" && isSupportTeamLead ? <SupportSubagents onInstruct={(subagent) => { setEditingInstructionId(null); setDiscussion([]); setInstructionTitle(""); setInstructionTarget(subagent); setInstructionPriority(2); setTab("Chat & Instruct"); }}/> : tab === "Subagents" && isFraudRiskAgent ? <FraudRiskSubagents onInstruct={(subagent) => { setEditingInstructionId(null); setDiscussion([]); setInstructionTitle(""); setInstructionTarget(subagent); setInstructionPriority(2); setTab("Chat & Instruct"); }}/> : tab === "Subagents" && isComplianceAgent ? <ComplianceSubagents onInstruct={(subagent) => { setEditingInstructionId(null); setDiscussion([]); setInstructionTitle(""); setInstructionTarget(subagent); setInstructionPriority(2); setTab("Chat & Instruct"); }}/> : tab === "Subagents" ? <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="font-bold">Subagents</h2><p className="mt-1 text-xs text-slate-500">Specialist agents reporting to {agent.name}. Select one to create or update its instructions.</p>{agent.subagents.length ? <div className="mt-5 grid gap-3 md:grid-cols-2">{agent.subagents.map((subagent, index) => <article key={subagent} className="flex items-center gap-3 rounded-xl border p-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-violet-50 text-[10px] font-bold text-violet-700">{String(index + 1).padStart(2,"0")}</span><span className="min-w-0"><b className="block text-xs">{subagent}</b><span className="text-[10px] text-slate-400">Specialist subagent</span></span><button onClick={() => { setEditingInstructionId(null); setDiscussion([]); setInstructionTitle(""); setInstructionTarget(subagent); setInstructionPriority(2); setTab("Chat & Instruct"); }} className="ml-auto h-8 shrink-0 rounded-lg border border-violet-200 px-3 text-[10px] font-bold text-violet-700">Instruct</button></article>)}</div> : <div className="mt-5 rounded-xl border border-dashed p-8 text-center text-xs text-slate-500">No specialist subagents are assigned to this department.</div>}</section>
    ...
    : tab === "Support Tickets" ? <SupportTickets action={action}/>
    : tab === "Risk Cases" ? <RiskCases action={action}/>
    : tab === "Review Queue" ? <RiskReviewQueue action={action}/>
    : tab === "Appeals" ? <RiskAppeals action={action}/>
    : tab === "Risk Rules" ? <RiskRules action={action}/>
    : tab === "KYC Cases" ? <ComplianceKycCases action={action}/>
    : tab === "Compliance Reviews" ? <ComplianceReviews action={action}/>
    : tab === "Financial Records" ? <ComplianceFinancialRecords action={action}/>
    : tab === "Ad Providers" ? <AdsProviders action={action}/>
    : tab === "Campaigns" ? <AdsCampaigns action={action}/>
    : tab === "Reward Tracking" ? <AdsRewardTracking action={action}/>
    : tab === "Revenue Reconciliation" ? <AdsRevenueReconciliation action={action}/>
    : tab === "Offer Providers" ? <OfferProviders action={action}/>
    : tab === "Surveys" ? <OfferSurveys action={action}/>
    : tab === "App-Install Offers" ? <AppInstallOffers action={action}/>
    : tab === "Task Tracking" ? <OfferTaskTracking action={action}/>
    : tab === "Conversion Reconciliation" ? <ConversionReconciliation action={action}/>
    : tab === "Providers" ? <AffiliateProviders action={action}/>
    : tab === "Product Feeds" ? <ProductFeeds action={action}/>
    : tab === "Product Catalog" ? <ProductCatalog action={action}/>
    : tab === "Stores & Deals" ? <StoresDeals action={action}/>
    : tab === "Click Tracking" ? <ClickTracking action={action}/>
    : tab === "Commissions" ? <AffiliateCommissions action={action}/>
    : tab === "Content Calendar" ? <ContentCalendar action={action}/>
    : tab === "Content Briefs" ? <ContentBriefs action={action}/>
    : tab === "Campaign Copy" ? <CampaignCopy action={action}/>
    : tab === "Content Sources" ? <ContentSources action={action}/>
    : tab === "Creative Requests" ? <CreativeRequests action={action}/>
    : tab === "Design Library" ? <DesignLibrary action={action}/>
    : tab === "Video Library" ? <VideoLibrary action={action}/>
    : tab === "Brand Review" ? <BrandReview action={action}/>
    : tab === "Approval Queue" && isCreativeProductionAgent ? <CreativeApprovalQueue action={action}/> : tab === "Approval Queue" ? <ContentApprovalQueue action={action}/>
    : tab === "Publishing Calendar" ? <PublishingCalendar action={action}/>
    : tab === "Scheduled Posts" ? <ScheduledPosts action={action}/>
    : tab === "Social Conversations" ? <SocialConversations action={action}/>
    : tab === "Content Performance" ? <ContentPerformance action={action}/>
    : tab === "Advertising Campaigns" ? <AdvertisingCampaigns action={action}/>
    : tab === "Audiences" ? <MarketingAudiences action={action}/>
    : tab === "Budgets" ? <MarketingBudgets action={action}/>
    : tab === "A/B Tests" ? <MarketingTests action={action}/>
    : tab === "Campaign Approvals" ? <CampaignApprovals action={action}/>
    : tab === "Tasks" && isPerformanceMarketingAgent ? <MarketingTasks/> : tab === "Tasks" && isSupportTeamLead ? <SupportTasks/> : tab === "Tasks" && isFraudRiskAgent ? <RiskTasks/> : tab === "Tasks" && isComplianceAgent ? <ComplianceTasks/> : tab === "Tasks" && isAdsOperationsAgent ? <AdsTasks/> : tab === "Tasks" && isOfferwallAgent ? <OfferTasks/> : tab === "Tasks" && isAffiliateShopAgent ? <AffiliateTasks/> : tab === "Tasks" && isContentManagerAgent ? <ContentTasks/> : tab === "Tasks" && isCreativeProductionAgent ? <CreativeTasks/> : tab === "Tasks" && isSocialMediaAgent ? <SocialTasks/> : tab === "Tasks" ? <AgentEmptyPanel icon={CheckCircle2} title="Agent Tasks" note="Running, waiting, completed, failed and escalated assignments will appear here." />
    : tab === "Reports" && isPerformanceMarketingAgent ? <MarketingReports/> : tab === "Reports" && isSupportTeamLead ? <SupportReports/> : tab === "Reports" && isFraudRiskAgent ? <RiskReports/> : tab === "Reports" && isComplianceAgent ? <ComplianceReports/> : tab === "Reports" && isAdsOperationsAgent ? <AdsReports/> : tab === "Reports" && isOfferwallAgent ? <OfferReports/> : tab === "Reports" && isAffiliateShopAgent ? <AffiliateReports/> : tab === "Reports" && isContentManagerAgent ? <ContentReports/> : tab === "Reports" && isCreativeProductionAgent ? <CreativeReports/> : tab === "Reports" && isSocialMediaAgent ? <SocialReports/> : tab === "Reports" ? <AgentEmptyPanel icon={SlidersHorizontal} title="Agent Reports" note="Daily, weekly and performance reports will appear here after activation." />
    : tab === "Approvals" && isPerformanceMarketingAgent ? <MarketingApprovals action={action}/> : tab === "Approvals" && isSupportTeamLead ? <SupportApprovals action={action}/> : tab === "Approvals" && isFraudRiskAgent ? <RiskApprovals action={action}/> : tab === "Approvals" && isComplianceAgent ? <ComplianceApprovals action={action}/> : tab === "Approvals" && isAdsOperationsAgent ? <AdsApprovals action={action}/> : tab === "Approvals" && isOfferwallAgent ? <OfferApprovals action={action}/> : tab === "Approvals" && isAffiliateShopAgent ? <AffiliateApprovals action={action}/> : tab === "Approvals" && isContentManagerAgent ? <ContentApprovals action={action}/> : tab === "Approvals" && isCreativeProductionAgent ? <CreativeApprovals action={action}/> : tab === "Approvals" && isSocialMediaAgent ? <SocialApprovals action={action}/> : tab === "Approvals" ? <AgentApprovalsShell agent={agent} action={action}/>
    : tab === "Automations" && isPerformanceMarketingAgent ? <MarketingAutomations action={action}/> : tab === "Automations" && isFraudRiskAgent ? <RiskAutomations action={action}/> : tab === "Automations" && isComplianceAgent ? <ComplianceAutomations action={action}/> : tab === "Automations" && isAdsOperationsAgent ? <AdsAutomations action={action}/> : tab === "Automations" && isOfferwallAgent ? <OfferAutomations action={action}/> : tab === "Automations" && isAffiliateShopAgent ? <AffiliateAutomations action={action}/> : tab === "Automations" && isContentManagerAgent ? <ContentAutomations action={action}/> : tab === "Automations" && isCreativeProductionAgent ? <CreativeAutomations action={action}/> : tab === "Automations" && isSocialMediaAgent ? <SocialAutomations action={action}/> : tab === "Automations" ? <SupportAutomations action={action}/>
    : tab === "Activity History" && isPerformanceMarketingAgent ? <MarketingActivity/> : tab === "Activity History" && isSupportTeamLead ? <SupportActivityHistory/> : tab === "Activity History" && isFraudRiskAgent ? <RiskActivityHistory/> : tab === "Activity History" && isComplianceAgent ? <ComplianceActivity/> : tab === "Activity History" && isAdsOperationsAgent ? <AdsActivityHistory/> : tab === "Activity History" && isOfferwallAgent ? <OfferActivity/> : tab === "Activity History" && isAffiliateShopAgent ? <AffiliateActivity/> : tab === "Activity History" && isContentManagerAgent ? <ContentActivity/> : tab === "Activity History" && isCreativeProductionAgent ? <CreativeActivity/> : tab === "Activity History" && isSocialMediaAgent ? <SocialActivity/> : tab === "Activity History" ? <AgentActivityHistoryShell agent={agent} instructions={instructions}/>
    : tab === "Connections" && isPerformanceMarketingAgent ? <MarketingConnections action={action}/> : tab === "Connections" && isSupportTeamLead ? <SupportConnections action={action}/> : tab === "Connections" && isFraudRiskAgent ? <RiskConnections action={action}/> : tab === "Connections" && isComplianceAgent ? <ComplianceConnections action={action}/> : tab === "Connections" && isAdsOperationsAgent ? <AdsConnections action={action}/> : tab === "Connections" && isOfferwallAgent ? <OfferConnections action={action}/> : tab === "Connections" && isAffiliateShopAgent ? <AffiliateConnections action={action}/> : tab === "Connections" && isContentManagerAgent ? <ContentConnections action={action}/> : tab === "Connections" && isCreativeProductionAgent ? <CreativeConnections action={action}/> : tab === "Connections" && isSocialMediaAgent ? <SocialConnections action={action}/> : tab === "Connections" ? <AgentEmptyPanel icon={Link2} title="Connections" note="Only approved email, provider, API and service connections will be available here." />
    : isPerformanceMarketingAgent ? <MarketingSettings/> : isSupportTeamLead ? <SupportSettings/> : isFraudRiskAgent ? <RiskSettings/> : isComplianceAgent ? <ComplianceSettings/> : isAdsOperationsAgent ? <AdsSettings/> : isOfferwallAgent ? <OfferSettings/> : isAffiliateShopAgent ? <AffiliateSettings/> : isContentManagerAgent ? <ContentSettings/> : isCreativeProductionAgent ? <CreativeSettings/> : isSocialMediaAgent ? <SocialSettings/> : <AgentPermissionsHierarchy agent={agent}/>}
...
function AgentEmptyPanel({ icon: Icon, title, note }: { icon: typeof Bot; title: string; note: string }) {
  return <section className="rounded-2xl border bg-white p-8 text-center shadow-sm"><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-violet-50 text-violet-600"><Icon size={21}/></span><h2 className="mt-4 font-bold">{title}</h2><p className="mx-auto mt-2 max-w-lg text-xs leading-5 text-slate-500">{note}</p><span className="mt-4 inline-flex rounded-full bg-slate-100 px-3 py-1 text-[9px] font-bold text-slate-500">NO DATA AVAILABLE</span></section>;
}
...
function AgentsCommandCenter({ action }: { action: (message: string) => void }) {
  ...
  return <>
    <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <div className="mb-2 flex items-center gap-2 text-xs text-slate-400"><Bot size={14} className="text-violet-600"/>AI Operations</div>
      ...
      <button onClick={() => action("The approved agent directory is managed in Agent Settings")} className="flex h-10 items-center gap-2 rounded-xl bg-violet-600 px-4 text-xs font-bold text-white shadow-lg shadow-violet-200">
    ...
    <section className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Agent platform summary">
      {[
        ["Main agents", "14", "Across all departments", Bot, "bg-violet-50 text-violet-700"],
        ["Subagents", "22", "Assigned specialists", Users, "bg-blue-50 text-blue-700"],
        ["Active tasks", "0", "No tasks in progress", CheckCircle2, "bg-emerald-50 text-emerald-700"],
        ["Need approval", "0", "No pending requests", ShieldCheck, "bg-amber-50 text-amber-700"],
    ...
    <section className="mb-5 rounded-2xl border border-violet-200 bg-violet-50 p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-white text-violet-700 shadow-sm"><ShieldCheck size={21}/></span><div><h2 className="text-sm font-bold text-violet-950">Controlled operations</h2><p className="mt-1 text-xs leading-5 text-violet-800">Agent activity follows approved instructions, role permissions, data-access boundaries and human approval rules.</p></div><span className="sm:ml-auto rounded-full border border-violet-200 bg-white px-3 py-1.5 text-[10px] font-bold text-violet-700">HUMAN OVERSIGHT</span></div>
    </section>

    <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b p-5 sm:flex-row sm:items-center"><div><h2 className="font-bold">Department Agents</h2><p className="mt-1 text-xs text-slate-500">Select a department to manage its instructions, tasks, approvals, reports and settings.</p></div><div className="sm:ml-auto flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2 text-[10px] font-bold text-slate-500"><ShieldCheck size={14} className="text-emerald-600"/>Human approval required</div></div>
      <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-3">
        {agentDirectory.map((agent) => { const Icon = agent.icon; return <button key={agent.number} onClick={() => setSelectedAgent(agent)} className="group flex min-h-[104px] items-center gap-4 rounded-xl border p-4 text-left hover:border-violet-300 hover:bg-violet-50/40">
          ...
          <span className="ml-auto rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-600 group-hover:bg-white">View</span>
        </button>})}
      </div>
    </section>
  </>;
