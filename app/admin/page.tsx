...
type AgentWorkspaceTab = "Overview" | "Chat & Instruct" | "Subagents" | "Support Tickets" | "Saved Instructions" | "Tasks" | "Reports" | "Approvals" | "Automations" | "Activity History" | "Connections" | "Settings";
...
function AgentWorkspace({ agent, onBack, action }: { agent: AgentDirectoryItem; onBack: () => void; action: (message: string) => void }) {
  ...
  const visibleWorkspaceTabs = isSupportTeamLead ? supportAgentWorkspaceTabs : agentWorkspaceTabs;
  ...
  return <>
    ...
    {tab === "Overview" && isChiefOperations ? <ChiefOperationsOverview instructionsCount={instructions.length} onOpen={setTab} action={action}/> : tab === "Overview" && isSupportTeamLead ? <SupportTeamLeadOverview onOpen={(nextTab) => setTab(nextTab as AgentWorkspaceTab)}/> : tab === "Overview" ? <div className="space-y-5">
    ...
    : tab === "Tasks" && isSupportTeamLead ? <SupportTasks/> : tab === "Tasks" ? <AgentEmptyPanel icon={CheckCircle2} title="Agent Tasks" note="Running, waiting, completed, failed and escalated assignments will appear here." />
    : tab === "Reports" && isSupportTeamLead ? <SupportReports/> : tab === "Reports" ? <AgentEmptyPanel icon={SlidersHorizontal} title="Agent Reports" note="Daily, weekly and performance reports will appear here after activation." />
    : tab === "Approvals" && isSupportTeamLead ? <SupportApprovals action={action}/> : tab === "Approvals" ? <AgentApprovalsShell agent={agent} action={action}/>
    : tab === "Automations" ? <SupportAutomations action={action}/>
    : tab === "Activity History" && isSupportTeamLead ? <SupportActivityHistory/> : tab === "Activity History" ? <AgentActivityHistoryShell agent={agent} instructions={instructions}/>
    : tab === "Connections" && isSupportTeamLead ? <SupportConnections action={action}/> : tab === "Connections" ? <AgentEmptyPanel icon={Link2} title="Connections" note="Only approved email, provider, API and service connections will be available here." />
    : isSupportTeamLead ? <SupportSettings/> : <AgentPermissionsHierarchy agent={agent}/>}
