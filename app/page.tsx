...
import {
  ...
  Home, Landmark, LockKeyhole, Mail, MonitorPlay, Play, Search, ShieldCheck,
  ShoppingBag, Smartphone, Star, Store, Target, Timer, TrendingUp,
...
function GamesScreen({ notify }: { notify:(m:string)=>void }) {
  ...
  const games=[{name:"Puzzle Quest",goal:"Complete level 10",reward:"₹50",icon:"🧩",tag:"Trending"},{name:"Cricket League",goal:"Win 3 matches",reward:"₹35",icon:"🏏",tag:"New"},{name:"Word Master",goal:"Find 500 words",reward:"₹25",icon:"🔤",tag:"Trending"},{name:"Ludo Club",goal:"Win 5 games",reward:"₹40",icon:"🎲",tag:"Top paying"}];
  ...
  <div className="overflow-hidden rounded-2xl border border-[#ece9f2] bg-white">{visible.map(g=><button onClick={()=>notify("Game offer tracking coming soon")} key={g.name} className="flex w-full items-center gap-3 border-b border-[#f0edf5] p-4 text-left last:border-0"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-violet-100 to-pink-100 text-2xl">{g.icon}</span><span className="flex-1"><b className="block text-sm text-[#282133]">{g.name}</b><span className="text-xs text-slate-500">{g.goal}</span></span><span className="text-sm font-black text-violet-600">+{g.reward}</span></button>)}</div></div>;
}
