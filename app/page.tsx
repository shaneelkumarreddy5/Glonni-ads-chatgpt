  ...
  return (
    ...
    >
      ...
      {!isOnline && (
        <div
          role="status"
          className="mx-4 mb-4 flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900 md:mx-8 lg:mx-10"
        >
...
function HomeScreen({
  ...
  const preferredLabel = interests.length
    ...
    : "Explore all rewards";
  const maskedMobile = userMobile
    ? `+91 ••••••${userMobile.slice(-4)}`
    : "Mobile verified";
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-3 md:grid-cols-[1.35fr_.65fr]">
        <button
          ...
          className="flex items-center gap-4 rounded-2xl border border-[#ece9f2] bg-white p-4 text-left shadow-[0_8px_25px_rgba(30,20,60,.04)]"
        >
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-violet-100 text-lg font-black text-violet-700">
            {userName.trim().charAt(0).toUpperCase() || "G"}
          </span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-extrabold text-[#282133]">
              {userName}
            </span>
            <span className="mt-1 block text-xs text-slate-500">
              {maskedMobile} · Demo profile
            </span>
            <span className="mt-1 block truncate text-[10px] font-bold text-violet-600">
              Interested in {preferredLabel}
            </span>
          </span>
        ...
        <button
          onClick={() => open("kyc")}
          className="flex items-center gap-3 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-left"
        >
          <span className="grid h-11 w-11 place-items-center rounded-xl bg-white text-amber-600">
            <ShieldCheck className="h-5 w-5" />
          </span>
          <span className="flex-1">
            <b className="block text-sm text-amber-900">Account setup 25%</b>
            <span className="text-xs text-amber-700">Continue KYC preview</span>
          </span>
      ...
      <button
        onClick={() => open("earnings")}
        className="flex w-full items-center gap-3 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-left"
      >
        ...
        <span className="min-w-0 flex-1">
          <b className="block text-sm text-amber-900">
            Track every reward clearly
          </b>
          <span className="mt-0.5 block text-xs text-amber-700">
            2 pending · 1 approved · See expected credit times
          </span>
        </span>
