...
function corsHeaders(request: Request) {
  ...
  return {
    "access-control-allow-origin": origin && allowedOrigins.has(origin)
      ? origin : "https://glonni-ads-chatgpt.vercel.app",
...
Deno.serve(async (request: Request) => {
  ...
  if (Number.isFinite(contentLength) && contentLength > 2048) {
    return respond(request, 413, { error: "payload_too_large" });
  }
  ...
  const result = await fetch(`${supabaseUrl}/rest/v1/rpc/list_admin_communications`, {
    method: "POST",
    headers: {
  ...
  if (!result.ok) {
    ...
    return respond(request, forbidden ? 403 : 500, {
      error: forbidden ? "admin_permission_required" : "communications_unavailable",
    });
  }
