...
Deno.serve(async (request: Request) => {
  ...
  const providerCode = (request.headers.get("x-communication-provider") ?? "").trim().toLowerCase();
  ...
  if (!providerPattern.test(providerCode) || !eventPattern.test(webhookId) ||
  ...
  const result = await fetch(`${supabaseUrl}/rest/v1/rpc/ingest_communication_delivery_event`, {
    method: "POST",
    headers: {
